  ...
  return (
    <div className="h-full flex flex-col bg-gray-50 dark:bg-gray-900">
      ...
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4">
        <h1 className="text-2xl font-bold text-gray-800 dark:text-white">📋 Historial de Cajas</h1>
        <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Visualiza y gestiona todas las cajas cerradas</p>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          {/* Ordenar */}
          <div>
            <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Ordenar por</label>
            <select
              value={sortBy}
      ...
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          <div className="bg-purple-50 dark:bg-purple-900/20 rounded-lg p-4">
            <p className="text-sm text-purple-600 dark:text-purple-400 font-semibold">Sesiones</p>
            <p className="text-2xl font-bold text-purple-800 dark:text-purple-300">{summary.totalSessions}</p>
          </div>
          <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
            <p className="text-sm text-blue-600 dark:text-blue-400 font-semibold">Total Ventas</p>
            <p className="text-2xl font-bold text-blue-800 dark:text-blue-300">{formatCurrency(summary.totalSales)}</p>
          </div>
          <div className="bg-red-50 dark:bg-red-900/20 rounded-lg p-4">
            <p className="text-sm text-red-600 dark:text-red-400 font-semibold">Total Egresos</p>
            <p className="text-2xl font-bold text-red-800 dark:text-red-300">{formatCurrency(summary.totalExpenses)}</p>
          </div>
          <div className={`rounded-lg p-4 ${summary.totalDifference < 0 ? 'bg-orange-50 dark:bg-orange-900/20' : 'bg-green-50 dark:bg-green-900/20'}`}>
            <p className={`text-sm font-semibold ${summary.totalDifference < 0 ? 'text-orange-600 dark:text-orange-400' : 'text-green-600 dark:text-green-400'}`}>
              Diferencia Neta
            </p>
            <p className={`text-2xl font-bold ${summary.totalDifference < 0 ? 'text-orange-800 dark:text-orange-300' : 'text-green-800 dark:text-green-300'}`}>
              {formatCurrency(summary.totalDifference)}
            </p>
          </div>
          <div className="bg-teal-50 dark:bg-teal-900/20 rounded-lg p-4">
            <p className="text-sm text-teal-600 dark:text-teal-400 font-semibold">Dif. Absoluta</p>
            <p className="text-2xl font-bold text-teal-800 dark:text-teal-300">{formatCurrency(summary.totalDifferenceAbs)}</p>
          </div>
        </div>
      </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-100 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300">Sesión</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300">Fecha Cierre</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300">Usuario</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-gray-700 dark:text-gray-300">Inicial</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-gray-700 dark:text-gray-300">Ventas</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-gray-700 dark:text-gray-300">Egresos</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-gray-700 dark:text-gray-300">Esperado</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-gray-700 dark:text-gray-300">Contado</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-gray-700 dark:text-gray-300">Diferencia</th>
                    <th className="px-4 py-3 text-center text-xs font-semibold text-gray-700 dark:text-gray-300">Acción</th>
                  </tr>
                </thead>
      ...
      {/* Footer con Exportar */}
      {filteredSessions.length > 0 && (
        <div className="bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 px-6 py-4 flex justify-end">
          <button
            onClick={handleExportCSV}
            className="flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 dark:bg-green-700 dark:hover:bg-green-600 text-white font-semibold rounded-lg transition-colors"
          >
            <Download size={18} />
            Exportar a CSV
          </button>
